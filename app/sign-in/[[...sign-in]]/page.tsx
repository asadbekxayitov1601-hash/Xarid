// Clerk sign-in catch-all route. ADDITIVE + ENV-GATED.
//
// When Clerk is enabled (keys present), renders Clerk's <SignIn/> themed to
      ...
      <div className="w-full max-w-md">
        <header className="mb-6 text-center">
          <h1 className="text-2xl font-bold text-text-primary">
            {t(locale, "clerk_signin_title")}
          </h1>
          <p className="mt-1.5 text-sm text-text-secondary">
            {t(locale, "clerk_signin_subtitle")}
          </p>
        </header>
...
async function ClerkSignIn() {
  ...
  const SignIn = mod?.SignIn as React.ComponentType<{ appearance?: unknown }> | undefined;
  if (!SignIn) {
    const locale = await getLocale();
