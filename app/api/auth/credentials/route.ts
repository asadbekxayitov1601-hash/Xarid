...
// Web sign in / sign up with phone + password. Public signup is buyer-only:
// an ordinary consumer who orders groceries -> /catalog (a User with no
// organization). Stores are admin-managed (added in /admin), not self-onboarded,
// so there is no longer a "seller" signup path.
//
...
// Route by the user's actual account type: a user who already owns a SUPPLIER
// org (e.g. store staff onboarded by the admin) lands in the portal; everyone
// who signs up here is a buyer.
